...
//     () => 
//     `https://api.openweathermap.org/data/2.5/weather?q=${search.value}&units=metric&appid=`
//     );
...
const handleClick = () => {
    const formatted_search = input.value.trim().split(" ").join("+");
    console.log(formatted_search);
    search.value = formatted_search;
    input.value = "";
};
...
<template>
    ...
    <div v-else class="p-10">
        <h1 class="text-7xl">Oops ,w e can't find that city</h1> 
